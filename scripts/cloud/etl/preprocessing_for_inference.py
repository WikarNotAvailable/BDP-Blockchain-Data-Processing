...
def setup_iceberg_table(spark):
    spark.sql("""
    CREATE TABLE IF NOT EXISTS glue_catalog.bdp.scaled_features_inference (
    ...
    spark.sql("""
    CREATE TABLE IF NOT EXISTS glue_catalog.bdp.unscaled_features_inference (
        transaction_hash STRING,
        sender_address STRING,
        receiver_address STRING,
        block_timestamp_unscaled TIMESTAMP,
...
def encode_string_variables(string_cols, df):
    ...
    for col_name in string_cols:
        ...
        elif col_name != "transaction_hash" and col_name != "sender_address" and col_name != "receiver_address":
...
def convert_datetime_to_unixtime(datetime_cols, df):
    ...
    for col_name in datetime_cols:
        if col_name == "block_timestamp":
            df_converted = df_converted.withColumnRenamed("block_timestamp", "block_timestamp_unscaled")
            df_converted = df_converted.withColumn("block_timestamp", unix_timestamp("block_timestamp_unscaled"))
        else:
